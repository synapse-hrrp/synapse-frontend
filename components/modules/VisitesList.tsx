...
type Row = {
  ...
  patient_id: string; service_id: number;
...
export default function VisitesList({ contextLabel = "Réception" }:{ contextLabel?: string }) {
  ...
  const allow = {
    ...
    del:    canAny(["visites.write"]), // ajuste si tu veux interdire la suppression
  };
  ...
  async function load(){
    ...
    }catch(e:any){ setErr(e?.message||"Erreur de chargement"); setRows([]); setTotal(0); }
      ...
      <div className="overflow-auto rounded-xl border border-ink-100 bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-ink-50 text-ink-700">
            <tr>
              <Th>Date</Th><Th>Patient</Th><Th>Service</Th><Th>Motif</Th>
              ...
              <tr key={String(r.id)} className="border-t border-ink-100 hover:bg-ink-50/40">
                ...
                <Td className="text-right pr-3">
                  <div className="inline-flex items-center gap-1">
                    {allow.view && <Link href={`/visites/${r.id}`} className="icon-btn" aria-label="Détail"><Eye className="h-4 w-4" /></Link>}
                    {allow.edit && <Link href={`/visites/${r.id}/edit`} className="icon-btn" aria-label="Modifier"><Pencil className="h-4 w-4" /></Link>}
