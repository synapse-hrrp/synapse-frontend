...
import { getToken, me, listUsersPaginated, deleteUser } from "@/lib/api";
...
export default function UsersListPage() {
  ...
  useEffect(() => {
    const t = getToken();
    if (!t) { window.location.replace("/login?next=/users"); return; }
    me().catch(() => window.location.replace("/login?next=/users"));
  }, []);

  // Flash
  useEffect(() => {
    const f = sp?.get("flash");
    if (f === "created") setToast({ show: true, text: "Utilisateur créé avec succès." });
    if (f === "deleted") setToast({ show: true, text: "Utilisateur supprimé." });
    if (f) {
  ...
  async function load() {
    ...
    try {
      const payload: any = await listUsersPaginated({ page, per_page: PAGE_SIZE, search: q });
  ...
  async function onDelete(id: number) {
    ...
    try {
      ...
      setToast({ show: true, text: "Utilisateur supprimé." });
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <nav className="text-sm text-ink-600">
            <ol className="flex items-center gap-2">
              ...
              <li className="font-medium text-ink-900">Utilisateurs</li>
            </ol>
          </nav>
                ...
                <tr key={r.id} className="border-t border-ink-100 hover:bg-ink-50/40">
                  ...
                  <Td className="text-right pr-3">
                    <div className="inline-flex items-center gap-1">
                      <Link href={`/users/${r.id}`} className="icon-btn" aria-label="Détail"><Eye className="h-4 w-4" /></Link>
                      <Link href={`/users/${r.id}/edit`} className="icon-btn" aria-label="Modifier"><Pencil className="h-4 w-4" /></Link>
