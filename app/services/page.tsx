...
import { getToken, me, listServicesPaginated, deleteService } from "@/lib/api";
...
export default function ServicesListPage() {
  ...
  useEffect(() => {
    const t = getToken();
    if (!t) { window.location.replace("/login?next=/services"); return; }
    me().catch(() => window.location.replace("/login?next=/services"));
  }, []);

  // Flash
  useEffect(() => {
    const f = sp?.get("flash");
    if (f === "created") setToast({ show: true, text: "Service créé." });
    if (f === "deleted") setToast({ show: true, text: "Service supprimé." });
    if (f === "updated") setToast({ show: true, text: "Service mis à jour." });
    if (f) {
  ...
  async function load() {
    ...
    try {
      const payload: any = await listServicesPaginated({ page, per_page: PAGE_SIZE, search: q });
  ...
  async function onDelete(id: number) {
    ...
    try {
      ...
      setToast({ show: true, text: "Service supprimé." });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-ink-100 to-white text-ink-900">
      ...
      <SiteHeader
        title="Services"
        subtitle="Référentiel des services de l’hôpital"
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <nav className="text-sm text-ink-600">
            <ol className="flex items-center gap-2">
              ...
              <li className="font-medium text-ink-900">Services</li>
            </ol>
          </nav>
            ...
            <tbody>
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-ink-100 hover:bg-ink-50/40">
                  ...
                  <Td>{r.code || "—"}</Td>
                  <Td>
                    <span className={`rounded-full px-2 py-0.5 text-xs ${r.is_active ? "bg-congo-greenL text-congo-green" : "bg-ink-200 text-ink-700"}`}>
                      {r.is_active ? "Actif" : "Inactif"}
                    </span>
                  </Td>
                  <Td className="text-right pr-3">
                    <div className="inline-flex items-center gap-1">
                      <Link href={`/services/${r.id}`} className="icon-btn" aria-label="Détail"><Eye className="h-4 w-4" /></Link>
                      <Link href={`/services/${r.id}/edit`} className="icon-btn" aria-label="Modifier"><Pencil className="h-4 w-4" /></Link>
