...
type Reagent = { id:number; name:string; sku:string; uom:string; reorder_point:number|null };
...
export default function ReactifEditPage(){
  ...
  useEffect(()=>{ (async()=>{
    try{
      ...
      setF({name:d?.name??"", sku:d?.sku??"", uom:d?.uom??"unit", reorder_point: d?.reorder_point!=null?String(d.reorder_point):""});
    }catch(e:any){ alert("Chargement impossible : "+(e?.message||"inconnue")); }
  })(); },[id]);

  async function submit(e:React.FormEvent){
    e.preventDefault(); setBusy(true);
    try{
      await apiFetch(`/inventory/reagents/${id}`,{method:"PATCH",body:{
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      <form onSubmit={submit} className="rounded-xl border bg-white p-4 shadow-sm space-y-4">
        <h1 className="text-lg font-semibold">Modifier réactif</h1>
        ...
        <Field label="UoM *"><input required value={f.uom} onChange={e=>setF(v=>({...v,uom:e.target.value}))} className="input"/></Field>
